...
public class PerformanceTestList extends ArrayList<PerformanceTestBase> {
    private static <T> Iterable<T> orderBy(Iterable<T> iterable,
        Function<T, ? extends Comparable> selector) {
    ...
    public void printResults(String heading) {
        ...
        for (PerformanceTestBase test : sortedByTime) {
            ...
            if (fastest == null) {
                ...
                System.out.println(
                    String.format("%s took %dms (%.2f%% slower)", testName, millis, percentSlower));
            }
        }
    }
}
